...
def train(hyp, opt, device, tb_writer=None):
    ...
    test_path = data_dict['val'] # validation data
    test2_path = data_dict['test'] # test data
    # Freeze
    ...
    if rank in [-1, 0]:
        testloader = create_dataloader(test_path, imgsz_test, batch_size * 2, gs, opt,  # testloader
                                       hyp=hyp, cache=opt.cache_images and not opt.notest, rect=opt.rect_test, rank=-1,
                                       world_size=opt.world_size, workers=opt.workers,
                                       pad=0.5, prefix=colorstr('val: '))[0]
        test2loader = create_dataloader(test2_path, imgsz_test, batch_size * 2, gs, opt,  # testloader
                                       hyp=hyp, cache=opt.cache_images and not opt.notest, rect=opt.rect_test, rank=-1,
                                       world_size=opt.world_size, workers=opt.workers,
